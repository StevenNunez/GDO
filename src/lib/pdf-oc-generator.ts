...
import type { Project, Supplier } from "@/modules/core/lib/data";
...
const DEFAULT_PAYMENT_TERMS = '30 DÍAS';

/**
 * Arma el payload de la OC a partir de los objetos de dominio.
 *
 * Existe porque las dos pantallas que emiten órdenes (procesar cotización e
 * historial de OCs) aplanaban el proveedor y la obra por su cuenta, y las
 * copias se desincronizaron: ambas terminaron con el nombre de una obra
 * concreta escrito a mano. Con esto, ese mapeo vive en un solo lugar.
 */
export function buildOCData(params: {
  ocNumber: string;
  date: Date;
  supplier: Supplier;
  /** Obra activa. `null` se imprime como "Sin obra asignada". */
  ...
  return {
    ...
    project: project?.name ?? 'Sin obra asignada',
    file: project?.id.slice(0, 8).toUpperCase() ?? '—',
    items: params.items,
    totalNet: params.totalNet,
    paymentTerms: params.paymentTerms ?? DEFAULT_PAYMENT_TERMS,
...
const COLORS = {
  primary: '#00528B',     // Azul de cabeceras y tabla
...
export async function generateOCPDF(data: OCData): Promise<{ blob: Blob; filename: string }> {
  ...
  if (address) doc.text(address, MARGIN, y + 9);

  // Logo (Derecha)
  if (company.logo) {
    try {
      doc.addImage(company.logo, pageWidth - MARGIN - 40, y - 5, 40, 15);
    } catch { /* un logo corrupto no debe impedir generar la orden */ }
  }
  ...
  const drawSumRow = (label: string, value: number, isTotal = false) => {
      ...
      doc.text(formatCLP(value), totalValueX, finalY, { align: "right" });
      finalY += 6;
  };
  ...
  doc.text(`RAZÓN SOCIAL: ${company.name}`, colLeft, infoY);
  if (company.telefono) doc.text(`Fono: ${company.telefono}`, colRight, infoY);
  infoY += 5;
  if (company.rut) doc.text(`RUT: ${company.rut}`, colLeft, infoY);
  if (company.email) doc.text(`Email: ${company.email}`, colRight, infoY);
  infoY += 5;
  if (address) doc.text(`DIRECCIÓN: ${address}`, colLeft, infoY);
  infoY += 5;
  if (company.giro) doc.text(`GIRO: ${company.giro}`, colLeft, infoY);
  ...
  doc.setFontSize(8);
  doc.setFont("helvetica", "normal");
  doc.text(company.representanteLegal || data.createdByName, MARGIN + 5, finalY + 14);
  doc.setFont("helvetica", "bold");
  if (company.representanteCargo) doc.text(company.representanteCargo, MARGIN + 5, finalY + 18);
  doc.text(company.name.toUpperCase(), MARGIN + 5, finalY + 22);
