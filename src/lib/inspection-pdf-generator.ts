...
function addHeader(doc: jsPDF, inspection: SafetyInspection, logo?: string) {
  ...
  // Logo
  if (logo) doc.addImage(logo, MARGIN, topY, logoSize, logoSize);
...
async function addSignatures(doc: jsPDF, y: number, inspection: SafetyInspection, supervisor: User, apr: User) {
  ...
  const addSignature = async (label: string, img: string | undefined | null, name: string, date: Date | string | null | undefined, x: number) => {
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(9);
...
export async function generateInspectionPDF(
  inspection: SafetyInspection, supervisor: User, apr: User,
