...
const getDate = (date: Date | string | null | undefined): Date =>
...
export async function generatePurchaseOrderPDF(order: PurchaseOrderType, supplier: Supplier, orderIndex: number) {
  ...
  if (company.logo) doc.addImage(company.logo, margin, y, 20, 20);
  ...
  if (company.rut) doc.text(`RUT: ${company.rut}`, margin, y + LINE_HEIGHT - 2);
  const address = companyAddressLine(company);
  if (address) doc.text(address, margin, y + (LINE_HEIGHT * 2) - 4);
