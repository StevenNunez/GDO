...
export default function PrintToolsQrPage() {
  ...
  // La etiqueta lleva el nombre de la empresa dueña de la herramienta.
  const [companyName, setCompanyName] = useState("");

  useEffect(() => {
    if (!user?.tenantId) return;
    getCompanyProfile(user.tenantId).then(p => setCompanyName(p.name));
  }, [user?.tenantId]);
  ...
  return (
    <div className="print-container flex flex-col gap-8">
      ...
      <SurfaceCard interactive={false}>
        <div className="p-4 md:p-6 print-content">
                             ...
                             <div key={tool.id} className="flex qr-item flex-col items-center justify-between text-center p-3 border-2 border-dashed rounded-lg aspect-[54/86] break-inside-avoid bg-background">
                                <div className='text-center'>
                                    ...
                                    {checkoutInfo.status === 'Ocupado' ? (
                                        <p className="qr-status-busy text-xs font-semibold text-danger">{checkoutInfo.status} por {checkoutInfo.workerName}</p>
                                    ) : (
                                        <p className="qr-status-free text-xs font-semibold text-success">{checkoutInfo.status}</p>
                                    )}
                                </div>
                                ...
                                <div className='text-center'>
                                    ...
                                    {companyName && (
                                        <p className="text-xs font-bold text-primary">{companyName.toUpperCase()}</p>
                                    )}
                                </div>
                            </div>
                        )
      ...
      <style jsx global>{`
        @media print {
          ...
          /* En papel los tokens no sirven: hay que fijar el color. Estos son los
             valores del tema claro (primario Prussian Blue, danger y success). */
          .qr-item .text-primary { color: #003F66 !important; }
          .qr-item .qr-status-busy { color: #c82929 !important; }
          .qr-item .qr-status-free { color: #26804f !important; }
