...
import { Loader2, Save, Calendar as CalendarIcon, Camera, Trash2, Users, Sparkles, MessageSquare } from "lucide-react";
...
import { suggestSafetyTalkTopic } from "@/actions/ask-assistant";
...
export default function DailyTalkPage() {
  ...
  const onSubmit: SubmitHandler<FormData> = async (data) => {
    ...
    try {
      ...
      const dataToSave = { 
        ...data,
        fecha: toDate(data.fecha)?.toISOString() ?? new Date().toISOString(),
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          <PanelCard
              title="Información de la Charla"
              description="Datos generales, expositor y temas tratados."
              icon={MessageSquare}
            ...
            >
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                        ...
                        <Controller name="obra" control={control} render={({ field }) => (
                            <Input id="obra" placeholder="Ej: File 721" {...field} />
                        )} />
                ...
                <div className="space-y-1.5">
                    ...
                    <div className="flex items-center gap-2 rounded-md border border-border bg-muted/40 px-3 py-2 text-sm">
                        ...
                        <span className="font-medium">{authUser?.name ?? 'Sin identificar'}</span>
                    </div>
                </div>

                <div className="space-y-1.5">
                    <div className="flex items-center justify-between">
                        <Label htmlFor="temas">Temas Tratados</Label>
                        <Button type="button" variant="outline" size="sm" onClick={handleSuggestTopic} disabled={isSuggesting}>
                            {isSuggesting ? <Loader2 className="mr-2 h-3.5 w-3.5 animate-spin" /> : <Sparkles className="mr-2 h-3.5 w-3.5 text-cta" />}
                            Sugerir Tema con IA
                        </Button>
                    </div>
                    <Controller name="temas" control={control} render={({ field }) => (
                        <Textarea id="temas" rows={4} placeholder="Describe los temas de seguridad tratados en la charla..." {...field} />
                    )} />
                    {errors.temas && <p className="text-xs text-destructive">{errors.temas.message}</p>}
                </div>
            </PanelCard>

            <PanelCard
              title="Foto de Evidencia"
              description="Adjunta una foto de la charla (opcional)."
              icon={Camera}
            >
                {evidencePhoto ? (
