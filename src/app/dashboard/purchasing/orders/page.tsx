...
const GenerateOrderCard: React.FC<GenerateOrderCardProps> = ({ lot, onArchive }) => {
    ...
    return (
        <PanelCard
            title={lot.category}
            description={`${totalRequests} ${totalRequests === 1 ? 'solicitud' : 'solicitudes'} • ${totalQuantity.toLocaleString()} unidades`}
            icon={PackagePlus}
            className="h-full"
            contentClassName="flex flex-col justify-end gap-4"
        >
                ...
                <div className="flex gap-2">
                    ...
                    <AlertDialog>
                        <AlertDialogTrigger asChild>
                             <Button
                                className="w-12 shrink-0 bg-success px-0 text-background hover:bg-success/90"
                        ...
                        <AlertDialogContent>
                            ...
                            <AlertDialogFooter>
                                ...
                                <AlertDialogAction onClick={handleArchiveWrapper} className="bg-success text-background hover:bg-success/90">
...
export default function OrdersPage() {
    ...
    const getDate = useCallback((date: Date | string | null | undefined): Date =>
        toDate(date) || new Date(date as any)
    , []);
        ...
        <div className="flex flex-col gap-8 fade-in pb-10">
            ...
            <div>
                ...
                {batchedLots.length > 0 ? (
                    ...
                    <div className="flex flex-col items-center justify-center rounded-3xl border-2 border-dashed border-border bg-card p-12 text-center text-muted-foreground">
                        <Inbox className="mb-4 h-12 w-12 opacity-50"/>
                        <h3 className="text-lg font-bold tracking-tight text-foreground">Todo al día</h3>
             ...
             <PanelCard
                ...
                description="Aquí puedes ver todas las solicitudes de cotización que has generado."
                icon={Truck}
                actions={
                    <Popover>
            ...
            >
                    <Accordion type="multiple" className="w-full space-y-4">
                        {filteredPurchaseOrders.length > 0 ? filteredPurchaseOrders.map((order, index) => (
                            <AccordionItem value={order.id} key={order.id} className="rounded-2xl border border-border bg-muted/40">
                            ...
                            <div className="flex flex-col items-center justify-center p-12 text-center text-muted-foreground">
                                ...
                                <h3 className="text-xl font-bold tracking-tight">Sin Cotizaciones</h3>
